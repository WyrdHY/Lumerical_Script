...
target  = n.^2-1;
para = lsqcurvefit(func,init_guess,x,target,[],[],opts);


%%
x_um = linspace(0.43,1.8,1000);
fitted = sqrt(func(para,x_um)+1);
figure; hold on;
plot(x, n, '--','DisplayName','Raw data');       % data
plot(x_um, fitted, 'r-','LineWidth',1.5,'DisplayName','Sellmeier fit'); % fit
%xlim([0.2 0.62]);
...
writematrix(out, fname, 'Delimiter','\t');   % tab-delimited text
